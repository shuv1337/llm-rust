...
pub use collection::{
    cosine_similarity, decode_embedding, delete_collection, encode_embedding, list_collections,
    Collection, EmbedItem, Entry,
};
pub use migrations::{
...
mod tests {
    ...
    fn test_encode_decode_roundtrip() {
        let original = vec![1.5f32, -2.0, 0.0, std::f32::consts::PI];
    ...
    fn test_collection_workflow() {
        ...
        let collection =
            Collection::in_memory("test_collection", "test-model").expect("create collection");
        ...
        collection
            ...
            .unwrap();
        collection
            .store("b", &[0.0, 1.0, 0.0], Some("east"), None)
            .unwrap();
        collection
            .store("c", &[0.0, 0.0, 1.0], Some("up"), None)
            .unwrap();
        ...
        let results = collection
            .similar_by_vector(&[1.0, 0.0, 0.0], 2, None)
            .unwrap();
    ...
    fn test_resolve_embedding_model_aliases() {
        ...
        assert_eq!(
            resolve_embedding_model("3-large"),
